			...
			<div class="card">
				...
				<div class="body">
					...
					<div class="form-group form-float">
						<div class="col-md-6">
							<a href="?page=add_retur&act=add" class="btn btn-success" ><i class="material-icons">add</i> Tambah Data</a>
						</div>
...
function tampil_list () {
		...
		$.ajax({
				type: 'POST',
				url: "functions/retur/show_retur.php",
				data: "startdate=" +startdate+ "&finishdate=" +finishdate,
				success: function(hasil) {
					$('.showretur').html(hasil);
				}
		});
}
...
$(document).on('click', '.hapusretur', function(){
		var no_retur = $(this).attr('data');
		//alert(no_retur);
		Swal.fire({
			title: 'CONFIRM',
			...
			if (result.value) {
				$.ajax({
							...
							data: "act=hapusretur&no_retur=" + no_retur ,
							dataType: "json",			
...
$(document).on('click', '.cetak', function(){
		...
		var link_base ="functions/retur/c_retur.php?no_retur="+no_retur+"&tgl_trans="+tgl_trans+"&tujuan="+tujuan+"&asal="+asal;
		$('#pdf_view').attr('src', link_base);
		$('#modal_Cetak').modal('show');
		$('.modal-title').text('Cetak Retur');
});
</script>
