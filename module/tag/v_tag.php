			...
			<div class="card">
				...
				<div class="body">
					...
					<div class="form-group form-float">
						<div class="col-md-6">
							<a href="?page=add_tag&act=add" class="btn btn-success" ><i class="material-icons">add</i> Tambah Data</a>
						</div>
...
function tampil_list () {
		...
		$.ajax({
				type: 'POST',
				url: "functions/tag/show_tag.php",
...
$(document).on('click', '.hapustag', function(){
		var no_tag = $(this).attr('data');
		//alert(no_tag);
		Swal.fire({
			title: 'CONFIRM',
			...
			if (result.value) {
				$.ajax({
							...
							data: "act=hapustag&no_tag=" + no_tag ,
							dataType: "json",			
...
$(document).on('click', '.cetak', function(){
		...
		var link_base ="functions/tag/c_tag.php?no_tag="+no_tag+"&tgl_trans="+tgl_trans+"&tujuan="+tujuan+"&asal="+asal;
		$('#pdf_view').attr('src', link_base);
		$('#modal_Cetak').modal('show');
		$('.modal-title').text('Cetak TAG');
});
</script>
