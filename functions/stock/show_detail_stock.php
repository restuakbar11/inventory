		...
		<tbody>
			...
			if($department==0) {
				$user2 =$mysqli->query( "SELECT IDITEM,NAMAITEM,IDSATUAN,NAMASATUAN,ITEMSTOCK_LOTNUMBER,ITEMSTOCK_ED,ID_DEPARTMENT,NAMA_DEPARTMENT,SUM(ITEMSTOCK_STOCK) AS STOCK
										...
										WHERE IDITEM='$item' AND IDSATUAN='$satuan' AND ITEMSTOCK_STOCK>0
			...
			while ($r =mysqli_fetch_array($user2)) { 
				...
				<tr>
					...
					<td><center>
						<a href="?page=stock&act=detailbarcode&item=<?php echo $r['IDITEM']?>&sat=<?php echo $r['IDSATUAN']?>&lot=<?php echo $r['ITEMSTOCK_LOTNUMBER']?>&ed=<?php echo $r['ITEMSTOCK_ED']?>&dept=<?php echo $r['ID_DEPARTMENT']?>" class="btn btn-info">Barcode <i class="fa fa-eye"></i></a>
						</center>
					</td>
				</tr>
				<?php
